...
function QuadraticDirect(Q::M, q::V) where {R <: Real, M <: DenseMatrix{R}, V <: AbstractVector{R}}
  QuadraticDirect{R, M, V, LinearAlgebra.Cholesky{R}}(Q, q)
end
...
function prox!(y::AbstractArray{R}, f::QuadraticDirect{R, M, V, F}, x::AbstractArray{R}, gamma::R=one(R)) where {R, M, V, F <: LinearAlgebra.Cholesky}
  if gamma != f.gamma
    factor_step!(f, gamma)
  end
  y .= x./gamma
