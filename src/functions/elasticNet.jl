...
function prox!(y::AbstractArray{R}, f::ElasticNet{R}, x::AbstractArray{R}, gamma::Real=one(R)) where {R <: Real}
  sqnorm2x = zero(R)
  norm1x = zero(R)
  gm = gamma*f.mu
  gl = gamma*f.lambda
...
function prox!(y::AbstractArray{Complex{R}}, f::ElasticNet{R}, x::AbstractArray{Complex{R}}, gamma::Real=one(R)) where {R <: Real}
  sqnorm2x = zero(R)
  norm1x = zero(R)
  gm = gamma*f.mu
  gl = gamma*f.lambda
