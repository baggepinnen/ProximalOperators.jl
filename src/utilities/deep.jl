...
deepcopy!(y::AbstractArray{R}, x::AbstractArray{R}) where {R <: Number} = copyto!(y, x)
...
function deepaxpy!(z::AbstractArray{T}, x::AbstractArray{T}, alpha::R, y::AbstractArray{T}) where {T <: Number, R <: Real}
    z .= x .+ alpha.*y
    return z
end
